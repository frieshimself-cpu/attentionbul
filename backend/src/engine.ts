...
/** Claim fees (only past the worth-claiming floor) and add 100% to the spam budget. */
async function claimIntoBudget(treasury: Keypair, state: BotState): Promise<bigint> {
  ...
  if (claimed > 0n) {
    creditBuckets(state, { spam: claimed }); // 100% funds new-pair spam
...
export async function runSpamEngine(
  ...
  for (;;) {
    try {
      ...
      // balance so the engine bursts off principal + rewards. Only ever raises the
      // budget; actual spending stays capped by the `affordable` guard, RESERVE kept.
