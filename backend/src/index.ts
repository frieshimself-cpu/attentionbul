...
import { config, lamportsToSol, solToLamports, validateLiveConfig, LEGEND_WALLETS } from './config.js';
...
import { buyBullpost } from './buyback.js';
import { feedTheLegends } from './legends.js';
...
const MIN_BUYBACK_LAMPORTS = solToLamports(0.02);
...
async function runCycle(creator: Keypair, state: BotState): Promise<void> {
  ...
  if (claimed >= config.minCycleLamports) {
    ...
    log(
      `split: +${lamportsToSol(split.pairSpam).toFixed(4)} pairSpam, ` +
        ...
        `+${lamportsToSol(split.legends).toFixed(4)} legends`
    );
    if (!config.dryRun) {
      ...
      ledger({ action: 'split', dryRun: true, claimed: claimed.toString(), split: { pairSpam: split.pairSpam.toString(), bagworkers: split.bagworkers.toString(), legends: split.legends.toString() } });
    }
  } else if (claimed > 0n) {
  ...
  if (!config.bagworkerWallet) {
    ...
    if (amount === 0n) {
      ...
      try {
        ...
        if (!config.dryRun) {
          creditBuckets(state, { pairSpam: 0n, bagworkers: amount, legends: 0n });
          saveState(state);
        }
      }
    }
  ...
  // 3. Feed the legends (25%): buy $BULLPOST, split it 4 ways.
  const legendsBucket = getBucket(state, 'legends');
  if (!config.bullpostMint) {
    if (legendsBucket > 0n) log('legends: BULLPOST_MINT not set — skipping buyback');
  } else if (legendsBucket >= MIN_BUYBACK_LAMPORTS) {
    const amount = legendsBucket <= (await spendable(creator)) ? legendsBucket : 0n;
    if (amount === 0n) {
      log('legends: wallet balance below bucket + reserve — deferring');
    } else {
      if (!config.dryRun) {
        debitBucket(state, 'legends', amount);
        saveState(state);
      }
      try {
        const bought = await buyBullpost(creator, amount);
        await feedTheLegends(creator, bought);
      } catch (err) {
        log(`legends step failed: ${(err as Error).message} — re-crediting bucket`);
        if (!config.dryRun) {
          creditBuckets(state, { pairSpam: 0n, bagworkers: 0n, legends: amount });
          saveState(state);
        }
      }
    }
  ...
  // 4. Pair spam engine (50%): launch billboards while the bucket affords them.
  ...
  while (
    ...
    try {
      ...
      if (!config.dryRun) {
        creditBuckets(state, { pairSpam: costPerLaunch, bagworkers: 0n, legends: 0n });
        saveState(state);
      }
...
function printStatus(state: BotState): void {
  log(
    `buckets: pairSpam ${lamportsToSol(getBucket(state, 'pairSpam')).toFixed(4)} SOL | ` +
      ...
      `legends ${lamportsToSol(getBucket(state, 'legends')).toFixed(4)} SOL`
  );
  log(
...
async function main(): Promise<void> {
  ...
  if (args.includes('--status')) {
    printStatus(state);
    log(`allocation: 50% pair spam / 25% bagworkers / 25% legends (${LEGEND_WALLETS.map((l) => l.name).join(', ')})`);
    return;
  }
